...
public class PlugableFrame extends JFrame {
        ...
        for (ViewPlugin plugin : plugins) {
            ...
            for (ActionDescriptor actionDescriptor : actionDescriptors) {
                ...
                if (action != null) {
                    ...
                    // Normalize accelerators for macOS: use Cmd instead of Ctrl
                    try {
                        if (com.ramussoft.gui.common.PlatformUtils.isMac()) {
                            KeyStroke ks = (KeyStroke) action.getValue(Action.ACCELERATOR_KEY);
                            if (ks != null) {
                                int ctrlMasks = InputEvent.CTRL_DOWN_MASK | InputEvent.CTRL_MASK;
                                int mod = ks.getModifiers();
                                if ((mod & ctrlMasks) != 0) {
                                    int menuMask = com.ramussoft.gui.common.PlatformUtils.getMenuShortcutKeyMask();
                                    int newMod = (mod & ~ctrlMasks) | menuMask;
                                    action.putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(ks.getKeyCode(), newMod));
                                }
                            }
                        }
    ...
    private void createJMenu() {
        ...
        // Remap accelerators to platform menu shortcut on macOS (Cmd vs Ctrl)
        applyMacAcceleratorRemap(bar);
    }
    ...
    private void applyMacAcceleratorRemap(JMenuBar bar) {
        try {
            if (!com.ramussoft.gui.common.PlatformUtils.isMac()) return;
        } catch (Throwable t) {
            return;
        }
        int menuMask = com.ramussoft.gui.common.PlatformUtils.getMenuShortcutKeyMask();
        ...
        for (int i = 0; i < bar.getMenuCount(); i++) {
            ...
            if (m != null) remapMenuRecursive(m, menuMask, ctrlMasks);
        }
    }

    private void remapMenuRecursive(JMenu menu, int menuMask, int ctrlMasks) {
        for (int i = 0; i < menu.getItemCount(); i++) {
            JMenuItem it = menu.getItem(i);
            if (it == null) continue;
            KeyStroke ks = it.getAccelerator();
            if (ks != null) {
                int mod = ks.getModifiers();
                if ( (mod & ctrlMasks) != 0 ) {
                    int newMod = (mod & ~ctrlMasks) | menuMask;
                    KeyStroke nks = KeyStroke.getKeyStroke(ks.getKeyCode(), newMod);
                    it.setAccelerator(nks);
                    Action a = it.getAction();
                    if (a != null) a.putValue(Action.ACCELERATOR_KEY, nks);
                }
            }
            if (it instanceof JMenu) {
                remapMenuRecursive((JMenu) it, menuMask, ctrlMasks);
            }
        }
    }
